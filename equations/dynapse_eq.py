...
231028 Fredrik Sandin updated all conditional factors to work with sympy > 1.5.1.
"""
#neuron
def dynapse_eq():
    return{'model': '''
                    ...
                    dIahp/dt = (- Ithahp_clip - Iahp + 2*Io*int(Iahp<=Io)) / (tauahp *\
                    ...
                    Itau_clip = Itau*int(Imem>Io) + Io*int(Imem<=Io)  : amp
                    Ith_clip = Ith*int(Imem>Io) + Io*int(Imem<=Io)    : amp
                    Iin_clip = clip(I_syn_nmda+I_syn_ampa-I_syn_gaba_a+Iconst,Io, 1*amp) : amp
                    Iahp_clip = Iahp*int(Imem>Io) + Io*int(Imem<=Io)  : amp
                    Ia_clip = Ia*int(Imem>Io) + 2*Io*int(Imem<=Io)    : amp
                    Ithahp_clip = Ithahp*int(Iahp>Io) + Io*int(Iahp<=Io) : amp
                    ...
                    2*Io*int(I_syn_nmda<=Io))/(tau_syn_exc*((I_g_clip_syn_exc/I_syn_nmda)+1)) : amp

                    I_g_clip_syn_exc = Io*int(I_syn_nmda<=Io) + I_g_syn_nmda*int(I_syn_nmda>Io) : amp
                    I_tau_clip_syn_exc = Io*int(I_syn_nmda<=Io) + I_tau_syn_nmda*int(I_syn_nmda>Io) : amp
                    ...
                    2*Io*int(I_syn_ampa<=Io))/(tau_syn_exc2*((I_g_clip_syn_exc2/I_syn_ampa)+1)) : amp

                    I_g_clip_syn_exc2 = Io*int(I_syn_ampa<=Io) + I_g_syn_ampa*int(I_syn_ampa>Io) : amp
                    I_tau_clip_syn_exc2 = Io*int(I_syn_ampa<=Io) + I_tau_syn_ampa*int(I_syn_ampa>Io) : amp
                    ...
                    2*Io*int(I_syn_gaba_a<=Io))/(tau_syn_inh *((I_g_clip_syn_inh/I_syn_gaba_a)+1)) : amp

                    I_g_clip_syn_inh = Io*int(I_syn_gaba_a<=Io) + I_g_syn_gaba_a*int(I_syn_gaba_a>Io) : amp
                    I_tau_clip_syn_inh  = Io*int(I_syn_gaba_a<=Io) + I_tau_syn_gaba_a*int(I_syn_gaba_a>Io) : amp
                    ...
                    2*Io*int(I_syn_gaba_b<=Io))/(tau_syn_shunt*((I_g_clip_syn_shunt/I_syn_gaba_b)+1)) : amp

                    I_g_clip_syn_shunt = Io*int(I_syn_gaba_b<=Io) +\
                    I_g_syn_gaba_b*int(I_syn_gaba_b>Io) : amp  # DPI's gain factor

                    I_tau_clip_syn_shunt = Io*int(I_syn_gaba_b<=Io) + I_tau_syn_gaba_b*int(I_syn_gaba_b>Io) : amp
